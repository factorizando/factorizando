...
export default function GrafoPanoramaSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 520 145" width="100%" style={{ display: "block" }}>
      ...
      <text x="260" y="19" fill={ac} fontSize="10" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.12em">GRAFOFONÉTICA</text>
      <text x="260" y="30" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">fonema (sonido) → grafema (letra o combinación)</text>
      ...
      <text x="128" y="48" fill={az} fontSize="8.5" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">FONEMAS VOCÁLICOS</text>
      <text x="128" y="60" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">correspondencia 1 : 1 — sin ambigüedad</text>
      ...
      {vowels.map(({ label, color }, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={px + pillW / 2} y="80" fill={color} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle" fontWeight="600">{label}</text>
          </g>
        );
      })}

      <text x="128" y="104" fill={tema.sub} fontSize="8" fontFamily="Georgia,serif" fontStyle="italic" textAnchor="middle">a · e · i · o · u</text>
      <text x="128" y="117" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">cada vocal tiene una sola letra</text>
      <text x="128" y="129" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">la y puede ser vocal /i/ al final de diptongo</text>
      ...
      <text x="388" y="48" fill={vd} fontSize="8.5" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">FONEMAS CONSONÁNTICOS</text>
      <text x="388" y="60" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">representación múltiple — aquí está la dificultad</text>

      {/* 5 líneas de pares fonema → grafemas */}
      {cLines.map(({ color, text }, i) => (
        <g key={i}>
          <circle cx="270" cy={72 + i * 14} r="2.5" fill={color} opacity="0.7"/>
          <text x="278" y={76 + i * 14} fill={tema.sub} fontSize="8.5" fontFamily="'Figtree', system-ui, sans-serif">{text}</text>
        </g>
      ))}
    </svg>
  );
