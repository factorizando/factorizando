...
export default function GrafoSecuenciasSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 520 138" width="100%" style={{ display: "block" }}>
      <text x="260" y="11" fill={tema.muted} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle" letterSpacing="0.14em" fontWeight="600">SECUENCIAS CON U: MUDA VS. PRONUNCIADA</text>
      {secs.map(({ label, color, uStatus, uPronounced, sound, examples }, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + colW / 2} y="32" fill={color} fontSize="10" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">{label}</text>
            {/* U status badge */}
            <rect x={x + 18} y="42" width={colW - 36} height="15" rx="4" fill={uPronounced ? `${color}35` : `rgba(255,255,255,0.06)`} stroke={color} strokeWidth="1"/>
            <text x={x + colW / 2} y="53.5" fill={uPronounced ? color : tema.muted} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle">{uStatus}</text>
            {/* Sound */}
            ...
            {examples.map(([word, pron], j) => (
              <g key={j}>
                ...
                <text x={x + colW / 2} y={100 + j * 17} fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">{pron}</text>
              </g>
            ))}
          </g>
        );
