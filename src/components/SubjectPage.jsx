...
function ResourcePill({ type, href, label, onClick }) {
  const styles = {
    quiz: {
      ...
      color: "#fff",
      border: "none",
      shadow: "0 2px 12px rgba(59,158,255,.3)",
    },
    video: {
      bg: "#1a2535",
      color: "#60a5fa",
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: ".6rem", flexWrap: "wrap", alignItems: "center", paddingLeft: ".6rem" }}>
        {subtopic.quiz && (
          <ResourcePill type="quiz" href={subtopic.quiz} label="Cuestionario" onClick={onQuizOpen} />
        )}
        {subtopic.video && (
          <ResourcePill type="video" href={subtopic.video} label="Video" />
        )}
        {subtopic.teoria && (
          <ResourcePill type="teoria" href={subtopic.teoria} label="Teoría" />
        )}
    ...
    <div style={{ borderBottom: isLast ? "none" : "1px solid #252830" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: ".7rem" }}>
          ...
          <span style={{ fontSize: ".9rem", fontWeight: 500, color: "#e8eaf0" }}>{theme.name}</span>
          <span style={{
            fontSize: ".68rem", color: "#5a6070", background: "#1e2230",
            border: "1px solid #2e3340", borderRadius: "20px", padding: ".15rem .6rem",
          }}>
            {subtopicCount} {subtopicCount === 1 ? "subtema" : "subtemas"}
          </span>
        </div>
        <span style={{
          color: "#5a6070", fontSize: ".8rem", transition: "transform .3s",
...
function SubjectBlock({ subject, onQuizOpen }) {
  ...
  return (
    <div style={{ borderRadius: "6px", overflow: "hidden", border: "1px solid #252830" }}>
      <div
        onClick={() => setOpen(!open)}
        style={{
          display: "flex",
          ...
          padding: "1rem 1.4rem",
          cursor: "pointer",
          background: open ? "#16181f" : "#13151a",
          transition: "background .2s",
          userSelect: "none",
        }}
        onMouseEnter={(e) => { if (!open) e.currentTarget.style.background = "#16181f"; }}
        onMouseLeave={(e) => { if (!open) e.currentTarget.style.background = "#13151a"; }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: ".6rem" }}>
          <span style={{ fontSize: "1.1rem" }}>{subject.icon}</span>
          <div>
            <div style={{ fontSize: ".92rem", fontWeight: 600, color: "#e8eaf0" }}>{subject.name}</div>
            <div style={{ fontSize: ".72rem", color: "#5a6070", marginTop: ".2rem" }}>{themeCount} temas</div>
          </div>
        </div>
        <span style={{
          color: "#5a6070", fontSize: ".9rem", transition: "transform .3s",
          transform: open ? "rotate(180deg)" : "rotate(0deg)", display: "inline-block",
        }}>▾</span>
      </div>

      <div style={{ maxHeight: open ? "4000px" : "0", overflow: "hidden", transition: "max-height .5s ease" }}>
...
export default function SubjectPage({ level, subjects }) {
  ...
  // Al hacer click en un quiz, navega al selector de bloque de esa ruta
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 960, width: "100%", margin: "0 auto" }}>
          ...
          <h1 className="page-title" style={{
            fontFamily: "'DM Sans', sans-serif",
            fontSize: "2.4rem", fontWeight: 700, color: "#e8eaf0", marginBottom: ".5rem",
          }}>
            {levelLabel}
          </h1>
