...
export default function MarcadoresTemporalesSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 520 100" width="100%" style={{ display: "block" }}>
      <text x="28"  y="11" fill={vd} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">INICIO</text>
      <text x="256" y="11" fill={az} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">DESARROLLO</text>
      <text x="480" y="11" fill={ac} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">CIERRE</text>
      {/* Timeline */}
      ...
      {pts.map(({ x, label, sub, color }) => (
        <g key={x}>
          ...
          <text x={x} y={TY + 18} fill={tema.sub} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">{sub}</text>
        </g>
      ))}
      {/* Simultaneous track */}
      ...
      <text x="8" y="98" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif">simultaneidad</text>
    </svg>
  );
}
