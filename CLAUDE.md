...
Operational back-office for the tutoring business (not exam content). It runs on the **design system** (light theme, `fx.css` tokens, no green/red, no emoji): `AdminLayout.jsx` is the shell (brand bar + grouped sidebar → horizontal strip under 900px; Sora/Figtree, lucide icons) and `ui.jsx` holds the primitives (`Page`, `Card`, `Stat`, `Badge`, `Button`, `SearchField`, `Field`, `Modal`, `EmptyState`, `Medidor`). Icons come from `lucide-react` (installed earlier but unused until now — its first real use). The brand reuses `FxMarca` and the top bar mirrors the public header's background/height, so the two read as the same bar. The default tab is **Inicio** (`AdminInicio.jsx`), deliberately **client-safe**: a welcome, a student search and shortcuts, **no figures** (it may be shown while enrolling or charging a client). The internal metrics live in **Dashboard** (`AdminDashboard.jsx`). Tabs: Inicio, Dashboard, Solicitudes, Alumnos, Tutores, Estadísticas, Presentaciones, Cursos, Inscripciones, Cargos, Suscripciones. Each `Admin<Tab>.jsx` accepts an `embedded` prop to render without its own shell when hosted inside `Admin.jsx`; standalone at `/admin/<tab>`. **Migration is phased**: Inicio, Dashboard, Personas (Solicitudes/Alumnos/Tutores) and Contenido (Estadísticas and Presentaciones, inline in `Admin.jsx`; Cursos in `AdminCursos.jsx`) are on the new system; only the Cobranza tabs (Inscripciones/Cargos/Suscripciones) still ship their old dark `AdminHeader` look inside the light shell until their phase. The presentation catalog uses the subject accent via `obtenerTema(materia, "claro")` (the `:root` accents, legible on paper) and grades are a sequential single-hue meter (`Medidor`), never a green/red semaphore.
