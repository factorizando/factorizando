...
npm run integridad # Cross-reference every id/key used against its registry
npm run responsive # Drive a real browser at 320-1280px against the §3 checklist
```
...
**`sumar-descomponiendo`, `multiplicar-descomponiendo` y `dividir-descomponiendo` are the sixth/seventh/eighth React talleres** (`src/components/talleres/descomponer/`, content in `src/data/talleres/descomponer/`) — three independent catalog entries sharing one engine and shell; see `docs/TALLER_DESCOMPONER_NUMEROS.md`. They practise the *relation* between an operation's parts with one term hidden: `(a+b)+c=d` (complements and breaking a summand to pass the ten), `a×b = a×m + a×n` (distributive over an area model) and `a = b×q + r` (reconstruct the missing total, groups, quotient or remainder). The data layer is shared (`rangos.js` is the single knob; `suma.js`/`producto.js`/`division.js` are the generators), and so is the UI (`Shell.jsx`, `Escenario.jsx`, `lib/pasos.js`); each catalog entry is a thin root component in `TALLERES_REACT` and gets its own `localStorage` key via `crearRegistro(<id>)`. `pruebas.js` asserts each constructed answer and the `0 ≤ r < b` invariant. The figures never show the value being asked (in division the groups come out empty when the per-group count is hidden) — same rule as the measuring glasses. Dev-only previews at `/preview-{sumar,multiplicar,dividir}-descomponiendo`.
...
**Access is explicit and admin-granted** (`20260921000000_acceso_por_bloque.sql`). Registering no longer opens content: `profiles.estado_acceso` ∈ `pendiente|aprobado|rechazado` (default `pendiente`) plus a single `bloque` ∈ `preparatoria|universidad|regularizacion` (one block per account, assigned on approval). `ProtectedRoute` enforces, in order: session → `perfil_completo` → staff (`rol` admin/profesor bypass) → `estado_acceso = "aprobado"` → **suspensión** → `bloque === requiredNivel`. A non-approved account lands on `/cuenta-pendiente` (status/reason, "Volver a solicitar", "Corregir mis datos"); `AuthCard.rutaPostAuth`, `CompletarPerfil` and `Home` all route there. **Suspension is a separate layer** (`20260923010000_suspension_cuentas.sql`): `profiles.suspendido_en`, `suspendido_hasta`, `motivo_suspension`, `suspension_origen` (`pago|ban|curso|manual`) cut access to an already-approved account without touching `estado_acceso`; reactivating clears the four columns and `suspendido_hasta` expires on its own. Staff is exempt. It is a **navigation** gate (content ships in the JS bundle), not RLS. Managed from the **Cuentas** tab (`AdminCuentas.jsx`), which shows overdue `cargos`/expired `suscripciones`/finished `inscripciones` as manual context; a suspended account lands on `/cuenta-suspendida`. **The `bloque` is the only authorization axis for a student account** (tutors carry none): it is chosen at approval and can be changed later from **Cuentas** or the student ficha (`AdminAlumnoDetalle.jsx`); `ProtectedRoute` and every post-login destination read it live, so no other plumbing is needed. **`CompletarPerfil` asks the account type first** (alumno/tutor, stored in `profiles.tipo_solicitado`, `20260922040000_profiles_apellidos_tipo.sql`) and branches: the **tutor** only gives name, `apellidos`, phone and avatar; the **alumno** also gives school data **and up to two emergency contacts** (name, phone, `parentesco`). Completing the alumno profile **creates/updates the `alumnos` expediente** (`id = profile_id = account id`, `nivel` derived from age) so the contacts have somewhere to live and `/alumno` works once approved; rejecting an application deletes that expediente (cascade removes its contacts). `profiles.apellidos` is stored separately so the old guess-split (`partirNombre`) is only a fallback. **`profiles.nivel` still exists but no longer authorizes** — it's derived from `nivel_educativo` and kept for stats/catalog. The admin approves from the **Solicitudes** tab (`AdminSolicitudes.jsx`, which shows `tipo_solicitado` and preselects it); approving `regularizacion` also ensures the `alumnos` row (id = profile id, `nivel` inferred by age: `<12` primaria else secundaria). `/regularizacion` is `requiredNivel="regularizacion"`: students with that block run talleres recording their own `taller_sesiones` (`alumno_id = auth.uid()`); admin/profesor run free (no write) — the old student picker is gone. Applying the migration leaves **every existing account pending** until approved.

**Email templates** (`supabase/email-templates/`, pasted by hand into Supabase): `confirm-signup.html` and `reset-password.html`. Supabase doesn't read them from the repo. The reset template **must** render `{{ .Token }}` — the app recovers the password with a 6-digit code (`verifyOtp({ type: "recovery" })`), not a link. See that folder's `README.md` for the subject lines and the Site URL requirement.

**Tutors and the accounts/records split** (`20260921010000_tutores_cuenta.sql`, `20260922010000_alumno_tutor_estado.sql`). Two layers, not alternatives: **`profiles`** is the *account* (one row per login; students with an account, tutors, admins), and **`alumnos`** is the *student record* (who takes classes and is billed), which **may or may not** point to an account. `alumnos.profile_id` (nullable, UNIQUE, `20260922000000_alumnos_profile_id.sql`) makes that link explicit — `NULL` = student without an account (primaria, manual). The invariant `alumnos.id = profiles.id` is kept for account-holders so client code that filters by `session.user.id` still works. **A tutor is simply a `profiles` row with `rol='tutor'` and `relacion` (`padre|madre|tutor`)**; the old `tutores` table was folded in (`20260923020000_plegar_tutores_profiles.sql`) because every tutor registers, and `alumno_tutor.tutor_id` now references `profiles(id)`. **Identity is synced from the account to the expediente** by the `sync_alumno_identidad` trigger (`20260923030000_sync_identidad_alumno.sql`): `profiles` is the source for nombre, apellidos, teléfono and fecha_nacimiento, and the Alumnos form only edits nivel and medical data for account-holders. `alumno_tutor` is N:M **with `estado`** (`pendiente|activo|rechazado`): only an **active** link lets the tutor read the student's data (`alumnos_del_tutor()` filters `activo`), progress (`resultados`, `taller_sesiones`) and payments; medical data stays admin-only. The tutor finds a student by **exact email/phone** (`buscar_alumno_para_tutor`, no open directory) and requests the link (`solicitar_vinculo_tutor`); the **student confirms** from `/alumno` (`resolver_vinculo_tutor`). A student **without an account cannot confirm**: the admin links them directly (activo) from the ficha. Portal at `/tutor` (`requiredNivel="tutor"`); each tutorado opens at `/tutor/alumno/:id`. **Tutor operator mode**: for a student without an account, the tutor practices *in their name*. Talleres: `/tutor/alumno/:alumnoId/practicar/:id` mounts `TallerVer` with the student's id as `alumnoId`, and the RLS write policy on `taller_sesiones` only allows it for the tutor's own students. Cuestionarios: the ficha links to `/cuestionario/:id?alumno=<id>`; `QuestionarioGenerico` writes `resultados.user_id = alumnoId || auth.uid()`. **`resultados.user_id` holds the *alumno id*** (equal to the account id when there is one) — `resultados` has no FK, and the tutor RLS already compared it against `alumnos_del_tutor()`, so no extra column was needed (`20260922030000_resultados_tutor.sql`). The admin approves tutor registrations in **Solicitudes** (just sets `rol='tutor'` + `relacion`; no ficha) and manages relation/archive in **Tutores**. The old recursion gotcha (`20260922050000`) is gone with the table: `alumno_tutor_tutor_select` is now just `tutor_id = auth.uid()`, and `alumnos_del_tutor()` no longer joins anything. Full account lifecycle (registration → profile → approval → who can edit what) is documented in `docs/CUENTAS.md`. **Correcting data:** an approved account self-edits at `/mis-datos` (`CompletarPerfil` with `modo="editar"`, linked from `/alumno` and `/tutor`; it re-writes `profiles` and the expediente but does **not** touch `perfil_completo`/`tipo_solicitado`), and the admin edits identity from **Cuentas** or the student ficha via `CuentaEditModal` — both write `profiles`, which the trigger syncs down. The **email** lives in `auth.users` and is not editable in-app (Supabase dashboard only). A **nonexistent/typo'd email** never confirms, so it cannot log in; `AuthCard` maps `email_not_confirmed` (message + resend) and asks the email twice at signup, and the `admin-cuentas` Edge Function (service role, `supabase/functions/`) lets the admin list and delete those accounts from **Cuentas → «Sin confirmar»**. `supabase/perfil_al_confirmar.sql` (manual, run in the Supabase SQL editor) moves profile creation from signup to confirmation so an unconfirmed address stops leaving a ghost `profiles` row — its existing trigger name must be confirmed first.

### Admin panel (`src/pages/admin/`, `src/components/admin/`)
Operational back-office for the tutoring business (not exam content). It runs on the **design system** (light theme, `fx.css` tokens, no green/red, no emoji): `AdminLayout.jsx` is the shell (brand bar + grouped sidebar → horizontal strip under 900px; Sora/Figtree, lucide icons) and `ui.jsx` holds the primitives (`Page`, `Card`, `Stat`, `Badge`, `BadgeEstado`, `Button`, `SearchField`, `Field`, `Modal`, `EmptyState`, `Medidor`). Icons come from `lucide-react` (installed earlier but unused until now — its first real use). The brand reuses `FxMarca` and the top bar mirrors the public header's background/height, so the two read as the same bar. The default tab is **Inicio** (`AdminInicio.jsx`), deliberately **client-safe**: a welcome, a student search and shortcuts, **no figures** (it may be shown while enrolling or charging a client). The internal metrics live in **Dashboard** (`AdminDashboard.jsx`). Tabs: Inicio, Dashboard, Solicitudes, Alumnos, Tutores, Cuentas, Estadísticas, Presentaciones, Cursos, Inscripciones, Cargos, Suscripciones. Each `Admin<Tab>.jsx` accepts an `embedded` prop to render without its own shell when hosted inside `Admin.jsx`; standalone at `/admin/<tab>`. **Migration is phased**: Inicio, Dashboard, Personas (Solicitudes/Alumnos/Tutores/Cuentas, plus the ficha de alumno in `AdminAlumnoDetalle.jsx`), Contenido (Estadísticas and Presentaciones, inline in `Admin.jsx`; Cursos in `AdminCursos.jsx`) and Cobranza (Inscripciones/Cargos/Suscripciones) are all on the new system. `AdminHeader.jsx` survives only as the shell of `/regularizacion` (outside `/admin`), and the old `EstadoBadge.jsx` only for the `/alumno` pages. The presentation catalog uses the subject accent via `obtenerTema(materia, "claro")` (the `:root` accents, legible on paper) and grades are a sequential single-hue meter (`Medidor`), never a green/red semaphore.

The public header (`FxHeader.jsx`) takes a `usuario` prop (`{ nombre, avatarUrl, destino }`): with a session it replaces "Entrar"/"Crear cuenta" with a **round avatar button** linking to that role's workspace (`/admin`, `/tutor`, or the student's `bloque`); `Home.jsx` no longer draws floating Admin/Regularización buttons.
...
- (no `tutores` table) — a tutor is a `profiles` row with `rol='tutor'` and `relacion` (`20260923020000_plegar_tutores_profiles.sql`). `alumno_tutor` links `alumnos.id ↔ profiles.id` (the tutor account) as N:M with `estado`; `AdminAlumnoDetalle.jsx`'s `TutorPicker` links an *existing* tutor account to a student rather than duplicating one.
...
**Mixed deletion (2026-09-23).** `alumnos` carries `archivado_en TIMESTAMPTZ` (`20260923000000_archivado_alumnos_tutores.sql`); `NULL` = active. With billing history (an alumno with `cargos`) the panel **archives** (`archivado_en` + links set inactive + account `estado_acceso = 'rechazado'`, never deleting `profiles`); with none, it hard-deletes. Lists have a *Ver archivados* toggle and *Restaurar*. In the student ficha, removing a tutor deletes **only** `alumno_tutor`. Since tutors are accounts now, **Tutores only archives/suspends** (`estado_acceso='rechazado'` + deactivate links) — it never deletes `profiles`. `cargos.alumno_id` is `ON DELETE RESTRICT`, so the hard path is only offered when there are no cargos; the modals surface the `DELETE` error instead of swallowing it.
...
**Verify UI with `npm run responsive`.** It starts a Vite server, drives Playwright's
bundled Chromium over the public routes at 320/360/375/414/768/1280, and measures the part
of the §3 checklist in `docs/DISENO.md` that a machine can measure: nothing overflowing the
viewport, no control under 44px tall below the 900px touch cut, no text under 15px. The
build sees none of these — an overflow is a card 40px off-screen, not a syntax error, which
is how the Home shipped for months with the wordmark hidden below 420px and the exam grid
spilling 40px on a 320px phone. Text size is measured only on migrated screens (the ones
with `.fx-page`) and reported as a warning, not an error; the reason is the open
`--fx-caption-size` question logged on 2026-08-26. Dark theme, hue-only distinctions and
hand-written hexes are still eye work. Add a public route to `RUTAS` in
`scripts/verificar-responsive.mjs` when you build one; protected routes return the login
without a session, so they can't be measured this way.

**`minmax(<n>px, 1fr)` does not shrink.** In `repeat(auto-fit, …)` the track keeps its
minimum even when the container is narrower, so the content overflows to the right instead
of reflowing. Always write `minmax(min(<n>px, 100%), 1fr)`. This bit the Home's three grids
and `MateriaVer`'s list; `npm run responsive` is what catches the next one.
...
**Which theme goes where (decision, 2026-09-22).** The **light** theme covers the *product surfaces*: Home, the Admin panel, the onboarding/auth forms (`/login`, `/registro`, `/completar-perfil`, `/cuenta-pendiente`, `/nueva-contrasena`), and the tutor and student profiles (`/tutor`, `/alumno`). The **dark** theme is for the **content**: theory, presentations, quizzes and the workshop player. The content stays dark but is being **homogenized** against the same tokens; it is not being migrated to light. New product pages are written light from the start. The old `theme.css` (Inter, `#80c6ff`, `--app-bg`) is legacy — auth forms were migrated off it to `fx.css` on 2026-09-22; it remains only for stragglers (e.g. `CursoVer`, `AppHeader`).
