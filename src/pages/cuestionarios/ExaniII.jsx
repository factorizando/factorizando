...
function toLatexInner(s) {
  ...
  s = s.replace(/([a-zA-Z0-9\)\}])([\u2070\u00B9\u00B2\u00B3\u2074-\u2079\u207B\u207A\u207F\u1D43\u1D50]+)/g,
    (_,base,sup) => base+'^{'+[...sup].map(c=>SMAP[c]||c).join('')+'}');
  s = s.replace(/(?<![a-zA-Z\d$])([−\-]?[a-zA-Z\d]{1,3})\/([−\-]?[a-zA-Z\d]{1,3})(?![a-zA-Z\d])/g,
    (m,n,d) => /^[a-zA-Z]{2,}$/.test(n)||/^[a-zA-Z]{2,}$/.test(d) ? m : '\\frac{'+n+'}{'+d+'}');
  return s;
}
function toLatex(s) {
  const saved = [];
  const p = x => { saved.push(x); return '\x05'+(saved.length-1)+'\x06'; };
...
function splitMath(text) {
  ...
  const saved = []; const PR='\x01', PE='\x02';
  const prot = x => { saved.push(x); return PR+(saved.length-1)+PE; };
  const res  = x => x.replace(new RegExp(PR+'(\\d+)'+PE,'g'),(_,i)=>saved[i]);
  let s1 = s0.replace(/\$[\d\s,.]+(?=[^\d/]|$)/g, prot);
  s1 = s1.replace(/[a-zA-Z]+\/[a-zA-Z²³]+/g, prot);
  const out = []; let buf = '', math = false;
  for (const tok of s1.split(/(\s+|[,;:¿?¡!.]+)/)) {
    const m = hasMath(tok);
    if (m !== math) { if (buf) out.push({math, s:res(buf)}); buf=''; math=m; }
    buf += tok;
  }
  if (buf) out.push({math, s:res(buf)});
  return out.map(seg => ({...seg, latex: seg.math ? toLatex(res(seg.s)) : ''}));
}
...
function MathText({ children, style }) {
  ...
  return (
    <span style={style}>
      {segs.map((seg,i) =>
        seg.math
          ? <span key={i} dangerouslySetInnerHTML={{__html:ktx.renderToString(seg.latex,{throwOnError:false,displayMode:false})}}/>
...
const questions = [
  ...
  {
    id:12, block:1,
    instruccion: "Seleccione la opción que completa correctamente el sentido del enunciado.",
  ...
  {
    id:15, block:1,
    instruccion: "El editor de una revista académica solicita a sus colaboradores una reseña crítica de un libro recién publicado. ¿Cuál texto es el adecuado?",
    q: "",
    opts: [
  ...
  {
    id:16, block:1,
    instruccion: "Elija la opción que tiene un significado opuesto a la palabra resaltada con negritas.",
  ...
  {
    id:20, block:1,
    instruccion: "Elija el enunciado en el que se utilizan los signos de puntuación correctamente.",
    q: "",
    opts: [
...
export default function ExaniII() {
  ...
  const startExam = (modeKey) => {
    ...
    setQueue(qs); setAnswers({}); setCurrent(0);
    setSelected(null); setConfirmed(false);
    setTimeLeft(qs.length * 60); setTimerOn(true);
      ...
      <div style={{minHeight:"100vh",background:C.bg,paddingBottom:64}}>
        ...
        <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"44px 24px 36px",textAlign:"center",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative"}}>
            <span style={{display:"inline-block",background:C.blue+"22",color:C.blue,borderRadius:99,padding:"3px 14px",fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:16,fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:40,flexWrap:"wrap"}}>
              {[{label:"Reactivos",val:totalQ},{label:"Áreas",val:2},{label:"Tiempo total",val:`${totalQ} min`}].map(s=>(
        ...
        <div style={{maxWidth:760,margin:"0 auto",padding:"32px 16px"}}>
          ...
          <button onClick={()=>startExam("all")} style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"22px 16px",cursor:"pointer",textAlign:"center",marginBottom:28,fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s"}}
            ...
            <div style={{color:C.text,fontWeight:700,fontSize:15}}>Todas las áreas</div>
            <div style={{color:C.muted,fontSize:12,marginTop:4}}>{totalQ} reactivos · {totalQ} min</div>
          </button>

          <p style={{color:C.muted,fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",marginBottom:12,fontFamily:"'DM Sans',sans-serif"}}>O elige un área</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:10}}>
            {BLOCKS.map((bl,i)=>{
              const count = bl.range[1] - bl.range[0] + 1;
              return(
                <button key={i} onClick={()=>startExam(`block-${i}`)} style={{background:C.surface,border:`1px solid ${bl.color}44`,borderRadius:12,padding:"18px 14px",cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s"}}
                  ...
                  <div style={{color:bl.color,fontWeight:700,fontSize:13,lineHeight:1.35,fontFamily:"'DM Sans',sans-serif",marginBottom:6}}>{bl.label}</div>
                  <div style={{color:C.muted,fontSize:11}}>{count} reactivos · {count} min</div>
                </button>
              );
            })}
          </div>
  ...
  if (mode === "results") {
    ...
    const msg=pct>=90?"¡Excelente dominio del temario EXANI-II!":pct>=75?"¡Muy bien! Repasa los temas fallidos.":pct>=50?"Sigue practicando, vas por buen camino.":"Te recomendamos reforzar el temario.";
          ...
          {displayQs.map(q=>{
            ...
            return(
              <div key={q.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",marginBottom:12}}>
                ...
                {q.instruccion && <p style={{color:C.muted,fontSize:12,marginBottom:6,fontStyle:"italic",fontFamily:"'DM Sans',sans-serif"}}><MathText>{q.instruccion}</MathText></p>}
                {q.q && <p style={{color:C.text,fontSize:14,fontWeight:600,marginBottom:10,lineHeight:1.6,fontFamily:"'DM Sans',sans-serif"}}><MathText>{q.q}</MathText></p>}
                <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:10}}>
                  {q.opts.map(opt=>{
                    ...
                    return <div key={opt} style={{background:bg,border:bd,color:co,borderRadius:8,padding:"10px 14px",fontSize:13,fontFamily:"'DM Sans',sans-serif",lineHeight:1.6}}>{isOk?"✓ ":""}{isUser&&!isOk?"✗ ":""}<MathText>{opt}</MathText></div>;
                  })}
                </div>
  ...
  const passageTitle = q.passage===1 ? PASSAGE_1_TITLE : q.passage===2 ? PASSAGE_2_TITLE : null;
  const passageBody  = q.passage===1 ? PASSAGE_1       : q.passage===2 ? PASSAGE_2       : null;
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,paddingBottom:64}}>
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"24px 16px"}}>
        ...
        {passageBody && (
          <div style={{background:C.surface,border:`1px solid ${C.gold}44`,borderRadius:12,marginBottom:14,overflow:"hidden"}}>
            <div style={{background:C.gold+"18",padding:"8px 16px",borderBottom:`1px solid ${C.gold}33`,display:"flex",alignItems:"center",gap:8}}>
              <span style={{color:C.gold,fontSize:11,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"'DM Sans',sans-serif"}}>📄 {passageTitle} — Lea el texto</span>
            </div>
            <div style={{padding:"16px 20px",maxHeight:280,overflowY:"auto"}}>
              {passageBody.split("\n\n").map((para,i)=>(
                <p key={i} style={{color:C.dim,fontSize:13.5,lineHeight:1.8,marginBottom:10,textAlign:"justify",fontFamily:"'DM Sans',sans-serif",fontStyle:i===passageBody.split("\n\n").length-1?"italic":"normal"}}>{para}</p>
              ))}
            </div>
          </div>
        )}
        ...
        {/* Tarjeta de pregunta */}
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",marginBottom:16}}>
          {q.instruccion && (
            <p style={{color:C.muted,fontSize:13,fontStyle:"italic",marginBottom:q.q?10:0,fontFamily:"'DM Sans',sans-serif",lineHeight:1.5}}>
              <MathText>{q.instruccion}</MathText>
            </p>
          )}
          {q.q && (
            <p style={{color:C.text,fontSize:"clamp(14px,2.5vw,16px)",fontWeight:600,lineHeight:1.7,fontFamily:"'DM Sans',sans-serif",margin:0,whiteSpace:"pre-line"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
          {q.opts.map(opt=>{
            ...
            return(
              <button key={opt} onClick={()=>!confirmed&&setSelected(opt)} style={{background:bg,border:bd,color:co,borderRadius:9,padding:"13px 16px",fontSize:14,fontWeight:500,cursor:confirmed?"default":"pointer",textAlign:"left",transition:"all 0.15s",outline:"none",fontFamily:"'DM Sans',sans-serif",lineHeight:1.6}}>
        ...
        <div style={{marginTop:24,background:C.surface,borderRadius:12,padding:16,border:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
            {queue.map((fq,i)=>{
              ...
              return <button key={fq.id} onClick={()=>{setCurrent(i);setSelected(answers[fq.id]||null);setConfirmed(!!answers[fq.id]);}} style={{width:28,height:28,borderRadius:5,background:bg,color:co,border:bdr,cursor:"pointer",fontSize:11,fontWeight:isCurr?700:400,fontFamily:"'DM Sans',sans-serif"}}>{i+1}</button>;
            })}
          </div>
