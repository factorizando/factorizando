...
export default function MarcadoresReformulacionSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 520 100" width="100%" style={{ display: "block" }}>
      <text x="88"  y="10" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle" fontWeight="600" letterSpacing="0.1em">FUENTE</text>
      <text x="222" y="10" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle" fontWeight="600" letterSpacing="0.1em">MARCADOR</text>
      <text x="340" y="10" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle" fontWeight="600" letterSpacing="0.1em">REFORMULACIÓN</text>
      <line x1="5" y1="13" x2="515" y2="13" stroke="rgba(255,255,255,0.08)" strokeWidth="1"/>
      {rows.map(({ type, color, marker, src, dst, desc }, i) => {
        const y = 19 + i * 27;
        return (
          <g key={i}>
            <text x="5" y={y + 14} fill={color} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" letterSpacing="0.06em">{type}</text>
            <rect x="82" y={y} width="92" height="22" rx="5" fill="rgba(0,0,0,0.25)" stroke="rgba(255,255,255,0.12)" strokeWidth="1"/>
            <text x="128" y={y + 14} fill={tema.sub} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">{src}</text>
            ...
            <text x="305" y={y + 14} fill={color} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">{dst}</text>
