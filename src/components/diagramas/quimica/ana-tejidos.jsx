...
export default function AnaTejidosSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 290 120" width="100%" style={{ display: "block", maxHeight: 130 }}>
      <text x={145} y={12} textAnchor="middle" fill={tema.acento} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif">los cuatro tejidos animales</text>
      {paneles.map(({ lab, sub, c }, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x+31} y={88} textAnchor="middle" fill={T} fontSize="7.6" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="600">{lab}</text>
            <text x={x+31} y={99} textAnchor="middle" fill={mu} fontSize="6.4" fontFamily="'Figtree', system-ui, sans-serif">{sub}</text>
          </g>
        );
      })}
      <text x={145} y={114} textAnchor="middle" fill={ro} fontSize="6.8" fontFamily="'Figtree', system-ui, sans-serif">la sangre es tejido conectivo: transporta gases y defiende</text>
    </svg>
  );
}
