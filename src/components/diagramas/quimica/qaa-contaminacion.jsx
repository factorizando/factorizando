...
export default function QaaContaminacionSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 290 125" width="100%" style={{ display: "block", maxHeight: 135 }}>
      {/* lluvia ácida */}
      <text x={72} y={14} textAnchor="middle" fill={rojo} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700">LLUVIA ÁCIDA</text>
      <rect x={28} y={20} width={88} height={16} rx={6} fill={`${mu}40`} stroke={mu} strokeWidth="1.2" />
      <text x={72} y={31} textAnchor="middle" fill={T} fontSize="6.5" fontFamily="'Figtree', system-ui, sans-serif">SO₂ · NOₓ + H₂O</text>
      ...
      <text x={72} y={74} textAnchor="middle" fill={mu} fontSize="6.3" fontFamily="'Figtree', system-ui, sans-serif">ácidos → daña suelos,</text>
      ...
      <rect x={160} y={36} width={112} height={26} rx={3} fill={`${mu}4d`} stroke={mu} strokeWidth="1" />
      ...
      <text x={216} y={74} textAnchor="middle" fill={mu} fontSize="6.3" fontFamily="'Figtree', system-ui, sans-serif">suelo (ciudad)</text>
      {/* pie */}
      <text x={145} y={98} textAnchor="middle" fill={T} fontSize="7" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="600">Agua: contaminación física · química · biológica</text>
      <text x={145} y={112} textAnchor="middle" fill={mu} fontSize="6.6" fontFamily="'Figtree', system-ui, sans-serif">fuentes: industrial · urbana · agrícola</text>
    </svg>
  );
}
