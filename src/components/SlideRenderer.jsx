...
import { Bloque, EjesXY, GenDobleHelice, Vector } from "./diagramas/comun.jsx";
...
function EjemploEstudiantesTabla({ tema }) {
  const cols = [
...
function CuartilesSVG({ tema, data }) {
  const a = tema.acento, bl = tema.azul, mu = tema.muted;
  const vals = [...(Array.isArray(data) ? data : [])].sort((p, q) => p - q);
  const n = vals.length;
  const median = (arr) => {
