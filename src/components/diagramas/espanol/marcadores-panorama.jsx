...
export default function MarcadoresPanoramaSVG({ tema }) {
  ...
  return (
    <svg viewBox="0 0 520 158" width="100%" style={{ display: "block" }}>
      ...
      <text x="260" y="21" fill={tema.acento} fontSize="10" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.1em">MARCADORES TEXTUALES</text>
      <text x="260" y="32" fill={tema.muted} fontSize="7.5" fontFamily="'Figtree', system-ui, sans-serif" textAnchor="middle">guían al lector en las relaciones lógicas entre las ideas</text>
      {cats.map(({ col, row, label, color, items }) => {
        ...
        return (
          <g key={label}>
            ...
            <text x={x + colW / 2} y={y + 13} fill={color} fontSize="8" fontFamily="'Figtree', system-ui, sans-serif" fontWeight="700" textAnchor="middle" letterSpacing="0.12em">{label}</text>
            {items.map((item, j) => (
