...
Operational back-office for the tutoring business (not exam content). Tabs: Alumnos, Solicitudes, Tutores, Estadísticas, Presentaciones, Cursos, Inscripciones, Cargos, Suscripciones. Each `Admin<Tab>.jsx` accepts an `embedded` prop to render without its own `AdminHeader`/page shell when hosted inside `Admin.jsx`'s tab switcher; it's also reachable standalone at `/admin/<tab>`.
