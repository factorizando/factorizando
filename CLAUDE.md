...
**Mixed deletion (2026-09-23).** `alumnos` and `tutores` carry `archivado_en TIMESTAMPTZ` (`20260923000000_archivado_alumnos_tutores.sql`); `NULL` = active. With billing history (an alumno with `cargos`) or linked students (a tutor), the panel **archives** (`archivado_en` + links set inactive + account `estado_acceso = 'rechazado'`, never deleting `profiles`); with none, it hard-deletes. Lists have a *Ver archivados* toggle and *Restaurar*. In the student ficha, removing a tutor deletes **only** `alumno_tutor` — global removal lives in Tutores. `cargos.alumno_id` is `ON DELETE RESTRICT`, so the hard path is only offered when there are no cargos; both modals now surface the `DELETE` error instead of swallowing it.
