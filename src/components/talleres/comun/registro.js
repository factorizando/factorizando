// Registro local de los talleres de juegos, como fábrica: cada taller llama
// `crearRegistro("<id>")` y recibe sus funciones ya atadas a su propia llave.
//
...
export function crearRegistro(tallerId) {
  const clave = (alumnoId) => `factorizando:${tallerId}:${alumnoId || "libre"}`;
  ...
  function cargarRegistro(alumnoId) {
  ...
  function anotarIntento(alumnoId, { rango, juego, categoria, acerto }) {
    if (!categoria) return;
  ...
  // nada.
  function cerrarPartida(alumnoId, { juego, modo, rango, aciertos, errores, total }) {
    const d = leer(alumnoId);
  ...
  function categoriasFlojas(alumnoId, { rango = null, minimo = 1 } = {}) {
    const d = leer(alumnoId);
  ...
  function resumenPorJuego(alumnoId) {
  ...
  function exportarJSON(alumnoId) {
  ...
  function borrarRegistro(alumnoId) {
    try {
