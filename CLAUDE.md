...
**Tutors** (`20260921010000_tutores_cuenta.sql`). A tutor is a `profiles` row with `rol='tutor'` linked to their `tutores` record via `tutores.profile_id`. `alumno_tutor` is N:M, so one tutor can have many students — including **students without an account** (the admin creates a bare `alumnos` row with a random UUID; see `AdminAlumnos.jsx`). `alumnos_del_tutor()` (SECURITY DEFINER) plus read-only RLS give the tutor their students' progress (`resultados`, `taller_sesiones`) and payments (`inscripciones`, `cargos`, `pagos`, `suscripciones`, `pagos_suscripcion`); medical data and emergency contacts stay admin-only. Portal at `/tutor` (`requiredNivel="tutor"`) lists the tutorados; each opens at `/tutor/alumno/:id`. The admin approves tutor registrations in **Solicitudes** (choosing an existing `tutores` record or creating one) and can link/unlink accounts from **Tutores**.
